...
public class DefaultParser implements CommandLineParser {
    ...
    private void handleShortAndLongOption(final String hyphenToken) throws ParseException {
        ...
        if (token.length() == 1) {
            handleShortOption(hyphenToken, token);
        } else if (pos == -1) {
            handleLongOptionWithoutEqualSign(hyphenToken, token);
        } else {
            handleEqualSignOption(hyphenToken, token, pos);
        }
    }

    private void handleShortOption(final String hyphenToken, final String token) throws ParseException {
    ...
    private void handleLongOptionWithoutEqualSign(final String hyphenToken, final String token) throws ParseException {
        if (options.hasShortOption(token)) {
            handleOption(options.getOption(token));
        } else if (!getMatchingLongOptions(token).isEmpty()) {
            handleLongOptionWithoutEqual(hyphenToken);  // or other logic for matching long options
        } else {
            handleLongPrefixOption(hyphenToken, token);
        }
    }

    private void handleLongPrefixOption(final String hyphenToken, final String token) throws ParseException {
        ...
        if (opt != null && options.getOption(opt).acceptsArg()) {
            ...
            handleJavaPropertyOption(hyphenToken, token);
        } else {
            handleConcatenatedOptions(hyphenToken);
        }
    }

    private void handleJavaPropertyOption(final String hyphenToken, final String token) throws ParseException {
    ...
    private void handleEqualSignOption(final String hyphenToken, final String token, final int pos) throws ParseException {
        final String opt = token.substring(0, pos);
        final String value = token.substring(pos + 1);

        if (opt.length() == 1) {
            handleShortOptionWithEqual(hyphenToken, opt, value);
        } else if (isJavaProperty(opt)) {
            handleJavaPropertyWithEqual(opt, value);
        } else {
            handleLongOptionWithEqual(hyphenToken);
        }
    }

    private void handleShortOptionWithEqual(final String hyphenToken, final String opt, final String value) throws ParseException {
    ...
    private void handleJavaPropertyWithEqual(final String opt, final String value) throws ParseException {
